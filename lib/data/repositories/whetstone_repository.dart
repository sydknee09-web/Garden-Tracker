...
class WhetstoneStreakStatus {
  const WhetstoneStreakStatus({
...
class WhetstoneRepository {
  ...
  /// Sanctuary "today" boundary: day rolls at 4:00 AM local time.
  ...
  Future<WhetstoneItem> addItem(String title) async {
    final existing = await SupabaseService.executeWithRetry(
  ...
  Future<void> seedStarterHabits() async {
    ...
    final inserts = starterHabits
        .asMap()
        .entries
        .map((e) => {'user_id': userId, 'title': e.value, 'order_index': e.key})
        .toList();
    await SupabaseService.executeWithRetry(
      () => SupabaseService.client.from(_itemsTable).insert(inserts),
    );
  }
  ...
  Future<void> markComplete({
    ...
    await SupabaseService.executeWithRetry(
      () => SupabaseService.client.from(_completionsTable).upsert({
        'user_id': SupabaseService.userId,
        'item_id': itemId,
  ...
  /// DB-backed streak status (Option A freeze), from RPC functions.
  Future<WhetstoneStreakStatus> fetchStreakStatus() async {
    final streakRaw = await SupabaseService.executeWithRetry(
      () => SupabaseService.client.rpc(
        'get_whetstone_streak',
        params: {'p_user_id': SupabaseService.userId},
      ),
    );
    final graceRaw = await SupabaseService.executeWithRetry(
      () => SupabaseService.client.rpc(
        'get_whetstone_grace_active',
        params: {'p_user_id': SupabaseService.userId},
      ),
    );
    return WhetstoneStreakStatus(
      streak: (streakRaw as num?)?.toInt() ?? 0,
      graceActive: graceRaw == true,
    );
  }
}
