...
function ShedPageInner() {
  ...
  const fetchSupplies = useCallback(async () => {
    ...
    setSupplies(rows);
    setLoading(false);
  }, [user?.id, refetchTrigger, isFamilyView]);
  ...
  const backHref = categoryFilter ? `/shed?category=${categoryFilter}` : "/shed";
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
          {filteredSupplies.map((s) => {
            ...
            return (
              <Link
                key={s.id}
                href={`/shed/${s.id}${categoryFilter ? `?from=shed&category=${categoryFilter}` : "?from=shed"}`}
