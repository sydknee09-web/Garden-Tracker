...
- growing_notes: a thorough, practical "how to grow this" narrative ORGANIZED into exactly these five labeled sections. Start each section with the label followed by a colon, each label on its own line, and separate sections with a blank line. Do NOT use markdown bullets, headers, or asterisks — just the plain labels below:
  Soil: 3-4 sentences on soil type, drainage, pH, and amendments to work in at planting.
  Watering: 3-4 sentences on watering practice in depth — how often, how to judge moisture, and establishment vs. established needs (this is the depth beyond the short Sun/Water pills).
  Feeding: 3-4 sentences on fertilizing/feeding cadence and what to use.
  Pruning & Training: 3-4 sentences on pruning, pinching, staking/support, and any training.
  Pests & Disease: 3-4 sentences on the common pests and diseases to watch for and how to respond.
  Use the EXACT labels above. If a section genuinely doesn't apply to this plant, still include the label with one short sentence saying so rather than omitting it. Keep the practical depth — this is the richest care guidance you can assemble. Use empty string for the whole field ONLY if you cannot find growing guidance at all.
...
- propagation_notes: for each NON-SEED propagation method that applies to this plant (cuttings, division, layering, runners, grafting, etc.), give a brief 2-3 sentence note covering WHEN in the year to do it and the key technique. Concise practical guidance per method — NOT a step-by-step tutorial. Seed-starting/sowing guidance does NOT belong here (it lives in the sowing_method / sowing_depth / days_to_germination fields). Use empty string if this plant is grown only from seed or you cannot find non-seed propagation guidance.
- propagation_method: comma-separated list of EVERY propagation method a home gardener could realistically use for this plant, from this EXACT vocabulary only: Seed, Cutting, Division, Layering, Grafting, Bulb-Tuber division, Spore, Runner. List ALL that apply, not just the single most common one (e.g. "Seed, Cutting" for Tomato; "Division, Seed" for Hosta; "Runner, Seed" for Strawberry; "Cutting, Layering, Grafting" for many fruit trees). Use empty string only if you genuinely cannot determine any method.
- seed_saving_notes: a detailed "how to save seeds from this plant" narrative for seed-grown varieties. Cover when and how to harvest seed (ripeness cues to look for), cleaning/processing (wet-process for fleshy fruit vs dry-process for pods/heads), drying, recommended storage conditions, and typical seed viability/longevity in years. If the plant readily cross-pollinates, note that isolation or hand-pollination is needed for true-to-type seed. Preserve practical depth. Use empty string only if seed-saving genuinely does not apply (e.g. plants not grown from seed) or you cannot find guidance.
...
Characteristics:
For the well-established taxonomic and intrinsic fields below — family, genus, species, native_origin, toxicity, pollination_requirements (and the lifecycle/growth_form/plant_category tags above) — give your best authoritative answer from general botanical knowledge even if the specific source page doesn't list it; these are species-level facts that do not vary by seed brand. Use empty string for these only when the value is genuinely unknown or not meaningfully defined for this plant. For the remaining, more variable characteristics (disease susceptibility, wildlife value, invasiveness, etc.), keep using empty string when you cannot find reliable information rather than guessing.
