...
- [ ] Verify star/due-date inheritance on split (Test #2 from First Five)
- [ ] Write integration test for full Mallet flow
...
- [ ] Verify starred pebble jumps to slot #1 on pack (Test #3)
- [ ] Verify burned slot stays empty (Test #4)
...
- [ ] Verify midnight sweep (Test #5 from First Five)
...
- [ ] Wire real background `Image.asset()` once owner drops files into `assets/backgrounds/`
- [ ] Wire real Elias `Image.asset()` once owner drops files into `assets/elias/`
- [ ] Verify Dawn to Midday transition (Test #1 from First Five)
...
- [ ] Wire real background `Image.asset()` (see Phase 9 asset table)
- [ ] Wire real Elias `Image.asset()` (see Phase 9 asset table)
- [ ] Hearth burn/shatter animation tuned
...
- [ ] Haptics on key interactions (burn, split, pack)
