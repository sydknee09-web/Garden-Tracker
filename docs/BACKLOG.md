...
- **Detail-page swipe — remaining follow-ups (2026-06-09 audit; filter-aware fix shipped `6c756b6` 2026-06-10).** Filter-aware swipe (via the shared [`lib/swipeOrder.ts`](../src/lib/swipeOrder.ts) snapshot) now covers **Library** + **Packet vault-context**. Remaining: **(1) Shed `vault/shed/[id]`** still uses a global updated_at-desc order (not filter-aware, and ≠ its alphabetical display sort) — adopt `swipeOrder` (scope `"supplies"`) from `ShedView` for parity; Syd didn't flag it (milder), queued. **(2) Growing-instance page `garden/grow/[id]` has NO swipe at all** — add-swipe (needs an ordered instance set + entry context); different scope, deferred. **(3) Library recent-sort coupling:** if Library default flips to recent-first (VISION §8 parked discovery-sort gap), its A–Z deep-link *fallback* should flip too (the snapshot path already follows the displayed sort). **(4) Optional cohesion:** the swipe *gesture mechanics* (handleSwipeStart/End + arrow chevrons) are still inlined per detail page — a `useDetailSwipe` hook could unify them on top of the now-shared order helper. All non-blocking; fold into a future Vault/Garden cohesion pass.
