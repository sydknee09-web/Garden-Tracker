...
- **Detail-page swipe inventory findings (2026-06-09, from Syd's swipe-pattern audit).** Between-item nav swipe exists on **Library `vault/[id]`** (profiles, name A–Z + framed `md:` chevrons), **Shed `vault/shed/[id]`** (supplies, updated_at desc + chevrons), and now **Packet `vault/packets/[id]`** (context-aware). Two follow-ups: **(1) GAP — Growing instance page `garden/grow/[id]` has NO swipe / no prev-next** (the only detail page without it; the `GrowInstanceModal` version also lacks it). Inconsistent with its siblings; candidate to add (needs an ordered grow-instance set + entry-context like the packet page). **(2) Shared-hook extraction:** Library + Shed + Packet each inline the same swipe logic separately — extract a `useDetailSwipe` hook for cohesion. **(3) Minor:** Shed swipe order (updated_at desc) ≠ its alphabetical display sort; Library swipe (A–Z) matches today but if Library flips to recent-first (VISION §8 parked discovery-sort gap) the swipe order must flip too. All non-blocking; fold into a future Vault/Garden cohesion pass.
