...
- **Packet detail page (`/vault/packets/[id]`) — swipe between packets.** *Parked 2026-06-09; revised after fold-in evaluation.* (a) **Confirm-on-delete — ✅ SHIPPED `6f3fad4` 2026-06-09** (folded in per Syd; mirrors the Delete Plant Profile alertdialog). (b) **Swipe between packets — still queued (real UX decision, not a quick reuse).** Sibling detail pages (Vault profile, Shed product) have swipe prev/next, but the gesture mechanic is inlined in `shed/[id]` (not a shared hook) and — more importantly — the packet page has **two entry contexts** (Vault Seed Packets list = all packets vs. profile Packets tab = one variety's packets), so "what set does swipe traverse?" is ambiguous: global swipe from a profile context would jump the user to an unrelated variety's packet. Resolve the set/order question (global vs per-profile-when-entered-from-profile; created_at vs displayed sort) before building. ~60–90 min + a Syd UX call; fold into a future Vault polish chapter or the Desktop Responsive Audit. Optionally extract `shed/[id]`'s swipe handler to a shared hook at the same time (cohesion).
