...
- **Packet detail page (`/vault/packets/[id]`) polish follow-ups.** *Parked 2026-06-09 (packet-detail-pages ship `03eee40`+`72cc01f`; not greenlit for the initial ship).* (a) **Confirm-on-delete:** "Remove Packet" currently deletes immediately (matching the old Packets-tab sibling behavior) then navigates back — on a dedicated page a confirm step may be safer; revisit. (b) **Swipe between packets:** sibling detail pages (Vault profile, Shed product) have swipe prev/next; the packet page omits it. Add for parity if dogfood wants it. Both XS polish; fold into a future Vault polish chapter.
