...
**Created:** March 2026  
**Purpose:** Single reference for everything else noticed and recommended fixes beyond the current MVP. Use alongside [AUDIT.md](../AUDIT.md) and [MASTER_PLAN.md](../MASTER_PLAN.md).
...
| 2 | **Run First Five tests** | Regressions possible; plan marks Tests #1–#5 unverified | Run manually on device per [TESTING_CHECKLIST.md](../TESTING_CHECKLIST.md); mark pass/fail in MASTER_PLAN (Phase 5, 6, 8, 9) |
...
| 4 | **Confirm satchel slots for new users** | New users might have 0 rows in `satchel_slots` | Test: create a brand‑new account, open Satchel, tap Pack (or open Scroll first). If empty slots don’t appear or errors occur, ensure app or backend creates 6 slots (see §4 below) |
...
| 9 | **Mark Phase 9 / 12 asset tasks done** | Plan still shows “Wire real background / Elias” unchecked | In MASTER_PLAN Phase 9 and 12, check off “Wire real background Image.asset()” and “Wire real Elias Image.asset()” (assets are in place and wired) |
...
## 4. Dependency Spring Cleaning (defer until after Voyager Sanctuary Overhaul)

**Decision:** Do not upgrade major packages (flutter_riverpod, go_router, flutter_slidable) right before a feature push. Major upgrades often introduce breaking changes:

- **go_router 14 → 17:** Redirects and observers may change.
- **flutter_riverpod 2 → 3:** API changes.
- **flutter_slidable 3 → 4:** API changes.

**Action:** After the Voyager Sanctuary Overhaul is complete, run a dedicated "Dependency Spring Cleaning" sprint. Address discontinued transitive packages (build_resolvers, build_runner_core) when upgrading riverpod_generator / build_runner.
...
| 18 | **Satchel slot source** | Schema defines `seed_satchel_for_user(uuid)` but the signup trigger only creates a profile. The app currently upserts 6 empty slots (e.g. in repository) when needed. Either: (a) call `seed_satchel_for_user` from `ensure_profile()` so every user has 6 rows from signup, or (b) document that app-side creation of 6 slots on first Satchel/Scroll load is intentional. |
...
## 7. Future / Multi-device

| # | Item | Notes |
|---|------|------|
| 25 | **Cross-client cache invalidation** | Cache is not cleared when data changes on another client (e.g. web). For a solo-user sanctuary/habit tracker, eventual consistency is fine. Defer until multi-device support. |

---

## 8. Release path (Phase 14)
...
- [x] Git initialized, main + dev, first commit
- [ ] First Five tests run and results noted in MASTER_PLAN
- [ ] RLS verified with two test accounts
- [ ] New-user satchel slots verified (or backend slot creation added)
...
- [ ] Phase 9/12 asset tasks marked done in MASTER_PLAN
