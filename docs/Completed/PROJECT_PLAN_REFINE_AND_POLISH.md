...
| **C** | Weave lore into 5-step wizard | Bloats wizard; harder to maintain. |
...
**Already clarified:** The hammer is **not** in the Satchel list. User selects a boulder stone and taps **Whetstone** → opens **"Refine the Path"** overlay. The hammer is the cursor used in *that* screen to break the boulder down. Satchel stays uncluttered.

**Decision:** Locked. Hammer = Refine Path overlay. No hammer in Satchel.
...
**Decision:** _____________ (Pack this Journey recommended)
...
4. **Whetstone setup step:** Elias explains the Whetstone via **inline prompt text in the Whetstone intro sheet** (e.g. "What small ritual keeps you steady?"); the sheet contains the prompt and habit fields together. User adds 1–3 habits in a guided flow. Data feeds into `whetstoneProvider` (same as Whetstone screen). User can edit anytime from the Whetstone screen.
5. Final screen: "You're ready. Begin." → dismiss to empty Sanctuary.

**Copy:** Generate and review Elias speech bubbles. See `elias_dialogue.dart` for tone. Add Whetstone intro lines to `elias_dialogue.dart`.

**Files:** New `lib/features/onboarding/elias_intro_overlay.dart`. `first_run_provider.dart` — add `hasSeenEliasIntro`. Trigger from `entrance_screen.dart` or `sanctuary_screen.dart` on first mount. Reuse `whetstoneProvider.addItem()` for habit creation.

**External work:** None. Copy can be generated in code.

---

### 4.2 Step-by-Step Elias Speech (Wizard)

**Problem:** Prompt and input on same page. Not gamified. User doesn't read.

**Fix:** Elias says his line first. User taps "Continue" (or tap anywhere). **Then** input field appears. Never both at once.

**Flow per step:**
1. Screen: Elias prompt only. "Continue" button at bottom.
2. User taps Continue.
3. Screen: Input field (or toggle/list). No prompt visible, or prompt faded above.

**Files:** `climb_flow_overlay.dart` — each step. Add `_showPromptFirst` state per step. Two-phase render.
...
## 6. Phase 3 — Hammer / Whetstone Refine Flow

**Clarification:** Hammer is **not** in Satchel. User taps Whetstone from Satchel → overlay with **Sharpen Habits only**. Refine/Edit is on the Map (Peak Detail): user goes to Map → Peak Detail → tap node → Edit overlay opens. In that overlay, hammer is used to break boulder into pebbles.

**Existing:** Edit overlay exists. Ensure Whetstone choice overlay has correct flow and tail points at Whetstone icon (per FEATURE_RECOMMENDATIONS §3.5).
