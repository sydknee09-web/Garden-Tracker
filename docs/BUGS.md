...
**Last updated:** 2026-05-13 (U24 added — ended plants still showing in Active Garden)
...
| U24 | Active Garden view (Garden page) | User ended 7 plants but they still appear in Active Garden. Reported 2026-05-13. Active Garden query likely doesn't filter out grow instances where `ended_at IS NOT NULL` (or `status != 'active'`). Code surfaces to investigate: `src/components/ActiveGardenView.tsx` rendering + the parent fetch in `src/app/garden/page.tsx` (a grep for `select.*from.*grow` in `garden/page.tsx` returned no matches — fetch is likely in a hook or inside `ActiveGardenView.tsx` itself; trace from `ActiveGardenView` props back to the source query). Schema confirmed: `grow_instances` table has `ended_at`, `end_reason`, `status` columns per `docs/GROW_INSTANCE_PAGE_SPEC.md:13`. **Diagnosis pending — read the Active Garden query path next chat.** Possible related causes (not exhaustive): (a) query missing `ended_at IS NULL` filter; (b) query filters on `status='active'` but the endPlant flow doesn't update `status` (only sets `ended_at` + `end_reason`); (c) optimistic UI state holds onto ended rows until refetch; (d) `deleted_at` and `ended_at` conflated somewhere in the filter logic; (e) household-toggle / RLS view showing ended rows from a different scope. | TBD after diagnosis (read the query path, identify which cause applies) | TBD |
