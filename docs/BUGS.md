...
**Last updated:** 2026-05-16 (U24 ✅ CLOSED — Phase A instrumented build surfaced PGRST204 on bulk-end; root cause = `ended_at` column missing from prod Supabase; user applied catch-up DDL via Dashboard SQL Editor; retry succeeded. U25 captured for follow-up — My Plants cross-surface icon mismatch. Plus migration audit run: only ONE additional column gap found across all public tables — `user_settings.onboarding_completed_at` — also fixed via Dashboard. Root cause of both gaps: two migrations with identical timestamp prefix `20250330000000` → ordering ambiguity → silent skip. Captured as process bug.)
