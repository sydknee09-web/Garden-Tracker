...
**As of 2026-05-12 — Journal search shipped (`7482c59`, chunk 3.9 item #6). `a7dadb7` verified clean. 4 ships still need prod verification. 6 remaining current-build items in chunk 3.9.**
...
🟢 **`7482c59` (Journal search)** — search field on journal page, multi-token AND across note + type + plant name + variety + date, all three view modes. **Has known follow-up bugs:** U21 (two clear-X buttons on Android Chrome) + journal page flicker / FAB cutoff (under investigation via the debug log).
...
6. **Journal search.** ✅ **SHIPPED 2026-05-12 (`7482c59`).** Search input above view-toggle row in journal sticky header. Multi-token AND across note, entry_type (underscores stripped), plant name + variety, formatted date. All three view modes (table/gallery/timeline). Awaiting user prod verification. **Follow-up bugs from ship (BUGS.md):** U21 (two clear-X buttons on Android Chrome — needs greenlight to fix).
