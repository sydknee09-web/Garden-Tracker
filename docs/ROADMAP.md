...
🟢 **`7482c59` (Journal search)** — search field on journal page, multi-token AND across note + type + plant name + variety + date, all three view modes. Awaiting user prod verification.
...
6. **Journal search.** ✅ **SHIPPED 2026-05-12 (`7482c59`).** Search input above view-toggle row in journal sticky header. Multi-token AND across note, entry_type (underscores stripped), plant name + variety, formatted date. All three view modes (table/gallery/timeline). Awaiting user prod verification.
