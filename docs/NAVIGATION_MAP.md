...
| 2026-04-06 | Phase 0 (Universal Add): **Add task** from FAB on Vault, Garden, and Journal now runs the same post-success path as Home and Calendar — toast, client refetch (`refetch` / `refetchTrigger`), and `router.refresh()` on Vault so lists and server-backed UI stay in sync without leaving the page. |
