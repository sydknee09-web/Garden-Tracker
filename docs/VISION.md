...
- **Desktop layout — calendar oversized + bottom nav feels mobile-y + empty horizontal space** (raised 2026-05-08). User flagged that on web browser: (a) the calendar grid is oversized for the viewport (~280px-wide cells on 1920px screens), (b) shrinking the calendar leaves a lot of empty horizontal space, (c) the bottom nav bar feels phone-y on desktop. User wondered about floating months with fade-slide transitions. *My recommendation: two-column desktop layout — calendar capped at ~600-700px on the left, Upcoming Tasks list on the right, both visible without scroll. Standard utility-app pattern (Linear / Notion / Mealime / Apple Reminders). Solves the empty-space problem since the right column isn't empty — it shows what you came to do. Plus left sidebar nav on desktop (replaces bottom nav at `lg:` breakpoint), same five icons, standard utility-app pattern (Notion / Linear / Slack / Figma). Floating-months idea is ornamental for a utility app — the calendar isn't the focus, the tasks are.* **Likely Phase 4 (Navigation) + Phase 5 (Page goals) territory** — calendar resize, two-column layout, sidebar nav, and the modal-placement issue above are entangled (sidebar width affects column widths, etc.) and should be designed as one coherent pass rather than fixed ad-hoc per surface.
