...
- **Sharing UX / purpose narrowing** (raised 2026-05-17). User signal verbatim: *"i want to dig into the sharing functionality and the purpose of sharing within the app. i have a few conflicting ideas and id like to narrow down the use of 'sharing' and how it will integrate/appear in the app."* Scoped as own dedicated chat purpose (parallel to Chat A aesthetic sweep) per ROADMAP §6 2026-05-17 entry. **Already-locked constraints (do NOT re-litigate in the sharing chat):** Self/Household/Trusted-circle 3-layer audience (VISION §3 + ROADMAP §6 2026-05-08); invite-only forever, never public, never strangers; Level 3 trades ✅ with history (both inventories update); Level 4 paid txns ❌ never; Level 5 public marketplace ❌ never. **Open territory the chat will explore:** (a) household sharing UX — one shared view, per-person filters, attribution of who did what; (b) trusted-circle sharing UX — does "view my sister's garden" appear as a separate tab, a filter on existing pages, or something else; (c) gardening-clubs-as-large-groups vs sister-circle-as-1-3-people — same UX or split; (d) where Level 3 trades initiate + live in the app (Vault? Garden? own surface?); (e) notifications / activity feed for circle activity vs no notifications; (f) sharing as passive backdrop vs active surface (discoverability + entry points). Dependencies: closest existing chunk is §3.5 lifecycle (transfer/give-away/trade), 🟣 Phase 3-gated. Decisions made in the sharing chat will inform §3.5 + may inform §3.4 (beds-as-first-class — bed sharing implications) + may surface new chunks. *Recommended Phase 2 jobs-to-be-done framing for the chat: walk each layer (self/household/circle/club) by the jobs the user is trying to accomplish there, surface where conflicting ideas live, propose resolution options.* Decision deferred to the dedicated chat where user has bandwidth to articulate her conflicting ideas + walk through them.
