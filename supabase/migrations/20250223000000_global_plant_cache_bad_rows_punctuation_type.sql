-- Extend bad-row criteria: exclude rows where plant type is punctuation-only (e.g. "?", ".")
-- so they never appear in get_global_plant_cache_plant_types() / Quick Add suggestions.
...
AS $$
  ...
  WHERE (gpc.extract_data->>'type') ~ '^prod\d+$'
     ...
     -- type with no letter (e.g. "?", ".", "-") is not a valid plant name for suggestions
     OR (TRIM(COALESCE(gpc.extract_data->>'type', '')) != '' AND TRIM(gpc.extract_data->>'type') !~ '[a-zA-Z]');
$$;

COMMENT ON FUNCTION public.get_global_plant_cache_bad_row_ids() IS
  'Used by scripts/clean-bad-cache and by get_global_plant_cache_plant_types: product codes, pepper hot/sweet, general+empty, bad identity_key, and type with no letter (e.g. ? or .).';
